...
export const branchSchema = z.object({
  id: identifierSchema,
  name: z.string().catch('Unknown branch'),
...
export const barberUserSchema = z.object({
  ...
  name: z.string().catch('Unknown barber'),
...
export const serviceSchema = z.object({
  id: identifierSchema,
  name: z.string().catch('Unnamed service'),
...
export const queueUpdateSchema = z.object({
  ...
  { message: 'Provide at least one queue update field.' }
)
...
export const breakSchema = z.object({
  minutes: numberLikeSchema.refine(value => value > 0, 'Minutes must be greater than zero')
})
...
export const certificateCreateSchema = z.object({
  ...
  service_ids: z.array(identifierSchema).min(1, 'Select at least one service'),
  expires_at: optionalTextSchema,
